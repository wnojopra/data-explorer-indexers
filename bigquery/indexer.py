...
def _add_participant_table_to_participant_docs(storage_client, bucket_name, export_obj_prefix,
    ...
    if 'synthetic_genotypes' in table_name:
        ...
        table_ref.project_id = table_name.split('.')[0]
        table_ref.dataset_id = table_name.split('.')[1]
        table_ref.table_id = table_name.split('.')[2]
        ...
        logger.info('Reading rows for {}'.format(table_name))
        ...
        for row in rows:
            ...
            if participant_id in participant_docs:
                ...
                participant_docs[participant_id] = row
        logger.info('Completed reading rows for {}'.format(table_name))
        return participant_docs
...
def _add_field_to_mapping(properties, field_name, entry, time_series_vals, is_samples_table):
    if time_series_vals and not is_samples_table:
...
def create_mappings(es, index_name, table_name, fields, participant_id_column,
    ...
    for field in fields:
        ...
        entry = {}
        #print('Willy field name and type {} {}'.format(field_name, es_field_type))
...
def main():
    ...
    exclude_from_time_series = bigquery_config.get('exclude_from_time_series', [])
    ...
    indexer_util.bulk_index_docs_in_parallel(args.elasticsearch_url, index_name, participant_docs)
